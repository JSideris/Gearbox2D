...
`),globalLines:["let nextId = 1;"],onInitRaw:`(world)=>{
        ...
        }`,onTick:(e,n)=>{if(Math.random()<.08){let o=.1+Math.random()*.4,a=1;Math.random()<.5&&(a=-1);let s=.2+o*o*.8,i=.1+Math.random()*(s-.1),r=s*s/i,l=Math.random(),h,x=o;l<.1?(h=t.shapes.POINT,x=.01):l<.2?h=t.shapes.AABB:l<.6?h=t.shapes.BOX:h=t.shapes.CIRCLE;const y=M++;e.makeBody(y,{x:5-a*5,y:7.5,r:Math.PI/2*Math.random(),rs:(Math.random()-.5)*5,vx:(2+Math.random()*5)*a,vy:-8-Math.random()*2,type:t.bodyTypes.DYNAMIC_OBJECT,mass:x,linearDamping:0}).addFixture({shape:h,radius:h===t.shapes.BOX||h===t.shapes.AABB?r:s,height:h===t.shapes.BOX||h===t.shapes.AABB?i:0}),e.getBodyCount();let g=[];e.iterateBodies(c=>{c.y>10.5&&g.push(c)});for(let c of g)e.removeObject(c.id)}}}),new T({name:"⚠ Friction",key:"friction",description:["**Friction** is applied as the last step of collision resolution. It handles both **static** and **dynamic** friction, applied as impulses at the point of contact.","Take note of the blue impulse vectors on the platforms which are present when dynamic friction is being applied.","### Scenarios","1. **Reverse Roll**: A circle spinning counter-clockwise transfers its angular momentum to linear momentum upon contact.","2. **Forward Roll**: A spinning circle with no linear momentum begins rolling forward due to friction.","3. **Slide**: A box slides across the platform and grinds to a halt.","4. **Static vs Kinetic**: Two boxes slide down a ramp. The left box has high static friction and stops; the right has no static friction and keeps sliding."].join(`
...
`),onInitRaw:`(world) => {
        ...
        }`,onInit:e=>{B=1,t.debug.showAabbs=!1,t.debug.showForceVectors=!0,e.setGravity(0,10),pe=[],Be=null,W=null;const n=B++,o=e.makeBody(n,{x:5,y:1,type:t.bodyTypes.FIXED_OBJECT,color:"#ff4444"});o.addFixture({shape:t.shapes.CIRCLE,radius:.2});const a=B++;W=e.makeBody(a,{x:5,y:2.5,mass:.05,color:"#888888"}),W.addFixture(a,{shape:t.shapes.CIRCLE,radius:.4}),Be=e.createHingeJoint(B++,o,W,{worldAnchor:{x:5,y:1}});const s=2,i=8,r=6,l=12,h=(i-s)/l,x=.2,y=B++,g=e.makeBody(y,{x:s-h/2,y:r,type:t.bodyTypes.FIXED_OBJECT,color:"#aaaaaa"});g.addFixture({shape:t.shapes.BOX,width:h,height:.5});const c=B++,d=e.makeBody(c,{x:i+h/2,y:r,type:t.bodyTypes.FIXED_OBJECT,color:"#aaaaaa"});d.addFixture({shape:t.shapes.BOX,width:h,height:.5});let p=g;for(let b=0;b<l;b++){const f=B++,m=e.makeBody(f,{x:s+b*h+h/2,y:r,mass:.2,color:"#cd853f"});m.addFixture({shape:t.shapes.BOX,width:h*.9,height:x,categoryBits:4,maskBits:-5});const k=e.createSpringJoint(B++,p,m,{worldAnchor:{x:s+b*h,y:r},frequencyHz:4,dampingRatio:1});pe.push(k),p=m}const u=e.createSpringJoint(B++,p,d,{worldAnchor:{x:i,y:r},frequencyHz:4,dampingRatio:1});pe.push(u),W.applyImpulse(.2,0)},onTickRaw:`(world, dt) => {
            // 1. Increase mass
            if (massObject) {
...
`),onInitRaw:`(world) => {
        ...
        }`,onInit:e=>{B=1;const n=2,o=5,a=5,s=1.5,i=B++,r=e.makeBody(i,{x:5,y:5,type:t.bodyTypes.FIXED_OBJECT,color:"#888"});r.addFixture({shape:t.shapes.AABB,width:6.2,height:.2,maskBits:0});let l=null;Fe=null;for(let h=0;h<a;h++){const x=h%2===0?1:.5,y=B++,g=e.makeBody(y,{x:n+h*s,y:o,mass:x,color:`hsl(${h*60}, 70%, 60%)`});g.addFixture({shape:t.shapes.CIRCLE,radius:x});const c=e.createHingeJoint(B++,r,g,{worldAnchor:{x:n+h*s,y:o}});if(h===0)Fe=g;else{const p=((h-1)%2===0?1:.5)/x;e.createGearJoint(B++,l,c,p)}l=c}},onTickRaw:`(world, dt) => {
            if (engineHub) {
...
`),onInitRaw:`(world) => {
        ...
        }`,onInit:e=>{t.debug.showAabbs=!1,t.debug.showForceVectors=!1,B=1;const n=5,o=5,a=4,s=B++,i=e.makeBody(s,{x:n,y:o,type:t.bodyTypes.FIXED_OBJECT,color:"#ff4444"});i.addFixture({shape:t.shapes.CIRCLE,radius:.5});const r=B++;Z=e.makeBody(r,{x:n,y:o,mass:100,color:"rgba(255, 255, 255, 0.05)",angularDamping:.5}),Z.addFixture(r,{shape:t.shapes.CIRCLE,radius:a,maskBits:0}),e.createHingeJoint(B++,i,Z,{worldAnchor:{x:n,y:o}});const l=8,h=3.8,x=.8,y=()=>({x:(Math.random()-.5)*.3,y:(Math.random()-.5)*.3});for(let g=0;g<l;g++){const c=g/l*Math.PI*2,d=n+Math.cos(c)*h,p=o+Math.sin(c)*h,u=d-Math.cos(c)*.5,b=p-Math.sin(c)*.5,f=B++,m=e.makeBody(f,{x:u,y:b,mass:.5,color:`hsl(${g*360/l}, 70%, 60%)`});m.addFixture({shape:g%2===0?t.shapes.BOX:t.shapes.CIRCLE,width:.5,height:.5,radius:.25});const k=Z.worldToLocal({x:d,y:p});e.createDistanceJoint(B++,Z,m,{anchorA:k,anchorB:y(),length:x});const I=u-Math.cos(c)*x,A=b-Math.sin(c)*x,R=B++,O=e.makeBody(R,{x:I,y:A,mass:.5,color:`hsl(${g*360/l}, 70%, 50%)`});O.addFixture({shape:(g+1)%2===0?t.shapes.BOX:t.shapes.CIRCLE,width:.5,height:.5,radius:.25}),e.createDistanceJoint(B++,m,O,{anchorA:y(),anchorB:y(),length:x});const J=I-Math.cos(c)*x,ne=A-Math.sin(c)*x,H=B++,G=e.makeBody(H,{x:J,y:ne,mass:.5,color:`hsl(${g*360/l}, 70%, 40%)`});G.addFixture({shape:(g+2)%2===0?t.shapes.BOX:t.shapes.CIRCLE,width:.5,height:.5,radius:.25}),e.createDistanceJoint(B++,O,G,{anchorA:y(),anchorB:y(),length:x})}e.setGravity(0,9.81),Z.rs=.2},onTickRaw:`(world, dt) => {
            if (drum) {
...
`),onInitRaw:`(world) => {
            ...
            world.setGravity(0, 9.81);
        }`,onInit:e=>{t.debug.showAabbs=!1,t.debug.showForceVectors=!1,B=1;const n=5,o=3,a=2,s=12,i=[],r=.2,l=B++,h=e.makeBody(l,{x:n,y:o,mass:2,color:"#ff8888"});h.addFixture({shape:t.shapes.CIRCLE,radius:r,sFriction:.9,kFriction:.9}),$=h,Me=0,Re=-30;for(let c=0;c<s;c++){const d=c/s*Math.PI*2,p=n+Math.cos(d)*a,u=o+Math.sin(d)*a,b=B++,f=e.makeBody(b,{x:p,y:u,mass:.5,color:"#8888ff"});f.addFixture({shape:t.shapes.CIRCLE,radius:.2,sFriction:.9,kFriction:.9}),i.push(f),e.createSpringJoint(B++,h,f,{anchorA:{x:Math.cos(d)*r,y:Math.sin(d)*r},length:a-r,frequencyHz:4,dampingRatio:.5}),c>0&&e.createSpringJoint(B++,i[c-1],f,{length:2*a*Math.sin(Math.PI/s),frequencyHz:4,dampingRatio:.5})}e.createSpringJoint(B++,i[s-1],i[0],{length:2*a*Math.sin(Math.PI/s),frequencyHz:4,dampingRatio:.5});const x=B++;e.makeBody(x,{x:5,y:10,type:t.bodyTypes.FIXED_OBJECT,color:"#aaaaaa"}).addFixture({shape:t.shapes.BOX,width:15,height:2,sFriction:.9,kFriction:.9});const y=B++;e.makeBody(y,{x:-3.5,y:4.25,type:t.bodyTypes.FIXED_OBJECT,color:"#aaaaaa"}).addFixture({shape:t.shapes.BOX,width:2,height:10});const g=B++;e.makeBody(g,{x:13.5,y:4.25,type:t.bodyTypes.FIXED_OBJECT,color:"#aaaaaa"}).addFixture({shape:t.shapes.BOX,width:2,height:10}),e.setGravity(0,9.81)},onTickRaw:`(world, dt) => {
            if (rotator) {
                if (impulseTimer <= 0) {
...
`),onInitRaw:`(world) => {
    ...
    }`,onInit:e=>{e.clear(),t.debug.showAabbs=!1,_=1,q=null;const n=5,o=4.5,a=.8,s=7,i=2.5,r=256,l=512,h=1024,x=2048,y=4096,g=_++,c=e.makeBody(g,{x:n,y:o,type:t.bodyTypes.FIXED_OBJECT,color:"#888"});c.addFixture(g,{shape:t.shapes.CIRCLE,radius:.15,categoryBits:r,maskBits:0});const d=_++,p=e.makeBody(d,{x:n,y:o+a,type:t.bodyTypes.FIXED_OBJECT,color:"#ff4444"});p.addFixture(d,{shape:t.shapes.CIRCLE,radius:.1,categoryBits:r,maskBits:0});const u=_++;q=e.makeBody(u,{x:n,y:o,mass:50,color:"#aaa"}),q.addFixture(u,{shape:t.shapes.CIRCLE,radius:.8,categoryBits:l,maskBits:0,restitution:0}),e.createHingeJoint(_++,c,q,{worldAnchor:{x:n,y:o}});for(let b=0;b<s;b++){const f=b/s*Math.PI*2,m=Math.cos(f),k=Math.sin(f),I=f-Math.PI/2,A=2.6,R=.28,O=2.5,J=1,ne=Y=>{const He=n+m*A+-k*(Y*J/2),Qe=o+k*A+m*(Y*J/2),_e=_++,se=e.makeBody(_e,{x:He,y:Qe,r:I,mass:1,color:"#bbb"});se.addFixture(_e,{shape:t.shapes.BOX,width:R,height:O,categoryBits:h,maskBits:x,restitution:0,sFriction:0,kFriction:0});const ce=se.localToWorld({x:0,y:-O/2}),Pe=se.localToWorld({x:0,y:O/2});e.createHingeJoint(_++,q,se,{worldAnchor:ce}),e.createDistanceJoint(_++,q,se,{worldAnchor:Pe})};ne(-1),ne(1);const H=a*Math.sin(f)+Math.sqrt(i*i-Math.pow(a*Math.cos(f),2)),G=n+m*H,fe=o+k*H,Ee=_++,oe=e.makeBody(Ee,{x:G,y:fe,r:I,mass:.5,color:"#ddd"});oe.addFixture(Ee,{shape:t.shapes.BOX,width:.7,height:1,categoryBits:x,maskBits:h,restitution:0,sFriction:0,kFriction:0});const Je=(n+G)/2,V=(o+a+fe)/2,ve=Math.atan2(fe-(o+a),G-n)-Math.PI/2,Ae=_++,Ie=e.makeBody(Ae,{x:Je,y:V,r:ve,mass:.2,color:"#fff"});Ie.addFixture(Ae,{shape:t.shapes.BOX,width:.15,height:i,categoryBits:y,maskBits:0,restitution:0}),e.createHingeJoint(_++,Ie,p,{anchorA:{x:0,y:-i/2},anchorB:{x:0,y:0}}),e.createHingeJoint(_++,Ie,oe,{anchorA:{x:0,y:i/2},anchorB:{x:0,y:0}})}e.setGravity(0,0),q.rs=2},onTickRaw:`(world, dt) => {
        if (engineHub) {
...
`),onInitRaw:`(world) => {
    ...
    }`,onInit:e=>{e.clear(),Nt(),t.debug.showAabbs=!1,e.setGravity(0,9.8),e.setHasRestitution(!0),e.setHasFriction(!0);const n=5,o=6,a=4,s=5,i=.2,r=De++;e.makeBody(r,{x:n,y:o+s/2+i/2,type:t.bodyTypes.FIXED_OBJECT,color:"#664422"}).addFixture({shape:t.shapes.BOX,width:a+i*2,height:i});const l=De++;e.makeBody(l,{x:n-a/2-i/2,y:o,type:t.bodyTypes.FIXED_OBJECT,color:"#664422"}).addFixture({shape:t.shapes.BOX,width:i,height:s});const h=De++;e.makeBody(h,{x:n+a/2+i/2,y:o,type:t.bodyTypes.FIXED_OBJECT,color:"#664422"}).addFixture({shape:t.shapes.BOX,width:i,height:s});const x=(c,d,p,u=!1)=>{if(p>=we.length)return null;const b=we[p],f=De++,m=e.makeBody(f,{x:c,y:d,mass:b.mass,color:b.color});return m.addFixture({shape:t.shapes.CIRCLE,radius:b.radius,restitution:.2,sFriction:.5,kFriction:.3}),m&&(m.wantsEvents=!0,C.fruitIds.set(f,p),u&&(m.vx=(Math.random()-.5)*.1)),m};e.onCollisionStart=(c,d)=>{if(C.isGameOver)return;const p=C.fruitIds.get(c),u=C.fruitIds.get(d);if(p!==void 0&&u!==void 0&&p===u){if(C.mergingIds.has(c)||C.mergingIds.has(d))return;const b=p;if(b>=we.length-1)return;C.mergingIds.add(c),C.mergingIds.add(d);const f=e.getBodyById(c),m=e.getBodyById(d);if(f&&m){const k=(f.x+m.x)/2,I=(f.y+m.y)/2;setTimeout(()=>{e.removeObject(c),e.removeObject(d),C.fruitIds.delete(c),C.fruitIds.delete(d),C.mergingIds.delete(c),C.mergingIds.delete(d),x(k,I,b+1)&&(C.score+=we[b+1].score)},0)}}},Q=document.getElementById("debug-canvas"),e._onMouseMove&&Q.removeEventListener("mousemove",e._onMouseMove),e._onClick&&Q.removeEventListener("click",e._onClick);const y=c=>{if(C.isGameOver)return;const d=Q.getBoundingClientRect();C.mouseX=Bn(c.clientX-d.left);const p=we[C.nextFruitLevel].radius;C.mouseX=Math.max(n-a/2+p,Math.min(n+a/2-p,C.mouseX))},g=c=>{if(C.isGameOver){e.clear(),Nt(),Zt.onInit(e);return}const d=Date.now();d-C.lastDropTime>C.dropCooldown&&(x(C.mouseX,o-s/2-1,C.nextFruitLevel,!0),C.nextFruitLevel=Math.floor(Math.random()*5),C.lastDropTime=d)};e._onMouseMove=y,e._onClick=g,Q.addEventListener("mousemove",y),Q.addEventListener("click",g)},onTickRaw:`(world, dt) => {
        gearbox.debug.clearLabels();
...
`),onInitRaw:`(world) => {
    ...
    }`,onTick:(e,n)=>{if(!E)return;const o=150,a=100,s=100;if(X){let r=X.r-E.r;for(;r>Math.PI;)r-=Math.PI*2;for(;r<-Math.PI;)r+=Math.PI*2;const l=-.6,h=.6;r<l?(X.r=E.r+l,X.rs<E.rs&&(X.rs=E.rs)):r>h&&(X.r=E.r+h,X.rs>E.rs&&(X.rs=E.rs))}if(D){let r=D.r-E.r;for(;r>Math.PI;)r-=Math.PI*2;for(;r<-Math.PI;)r+=Math.PI*2;const l=-.1,h=.8;r<l?(D.r=E.r+l,D.rs<E.rs&&(D.rs=E.rs)):r>h&&(D.r=E.r+h,D.rs>E.rs&&(D.rs=E.rs))}if(ae.KeyD&&me.rs<s&&me.applyAngularImpulse(o*n),ae.KeyA&&me.rs>-s&&me.applyAngularImpulse(-o*n),ae.KeyW&&E.applyAngularImpulse(-a*n),ae.KeyS&&E.applyAngularImpulse(a*n),ae.KeyR){en.init(e);return}const i=ue[ue.length-1];if(i.x<E.x+25){const r=6+Math.random()*8,l=1.2,h=(Math.random()-.5)*.5,x=Math.random()<.2?1.5:0,y=i.localToWorld({x:i.fixtures[0].width/2,y:0}),g=y.x+x;let c=y.y+(x>0?(Math.random()-.5)*3:0);c=Math.max(2,Math.min(8,c));const d=Math.cos(h),p=Math.sin(h),u=g+r/2*d,b=c+r/2*p,f=S++,m=e.makeBody(f,{x:u,y:b,r:h,type:t.bodyTypes.FIXED_OBJECT,color:`hsl(${20+Math.random()*40}, 30%, ${30+Math.random()*20}%)`});if(m.addFixture({shape:t.shapes.BOX,width:r,height:l,categoryBits:ie,maskBits:Te|Se}),ue.push(m),ue.length>50){const k=ue.shift();e.removeObject(k.id)}}t.debug.clearLabels(),E.x<15&&(t.debug.addLabel({text:"Motorcycle Trials",x:5,y:3,fontSize:"28px Arial",color:"#fff",position:"on-top"}),t.debug.addLabel({text:"Use D/A to drive and W/S to balance!",x:5,y:3.5,fontSize:"16px Arial",color:"#aaa",position:"on-top"}))},onRender:e=>{if(!E)return;const n=e.interpolationAlpha,o=E.x*n+E.prevX*(1-n),a=E.y*n+E.prevY*(1-n),s=t.debug.canvas?.width||800,i=t.debug.canvas?.height||600,r=t.debug.zoom,l=s/2-o*100*r,h=i/2-a*100*r;t.debug.offsetX+=(l-t.debug.offsetX)*.1,t.debug.offsetY+=(h-t.debug.offsetY)*.1},onCleanup:e=>{const n=e._motorcycleListeners;n&&(window.removeEventListener("keydown",n.onKeyDown),window.removeEventListener("keyup",n.onKeyUp),re&&re.removeEventListener("mousedown",n.mouseDownHandler),window.removeEventListener("mousemove",n.mouseMoveHandler),window.removeEventListener("mouseup",n.mouseUpHandler),delete e._motorcycleListeners),be&&(e.removeJoint(be.id),be=null),j&&(e.removeObject(j.id),j=null),t.debug.offsetX=0,t.debug.offsetY=0}}),Tn=[Zt,fn,In,en],En=!["localhost","127.0.0.1"].includes(window.location.hostname),ge=[{name:"Showcase",examples:Tn},{name:"General",examples:un},{name:"Constraints",examples:yn},{name:"Optimizations",examples:gn},{name:"Load Tests",examples:xn},{name:"Known Issues",examples:bn}].filter(e=>!En||e.name!=="Known Issues");let vn=`
