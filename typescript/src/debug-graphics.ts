...
import { 
    World, Body, Fixture, HingeJoint, DistanceJoint, SpringJoint, GearJoint, Joint,
    SHAPES, IS_SLEEPING, HAS_PHYSICAL_COLLISION, HAS_AABB_COLLISION, JOINT_TYPES 
} from './gearbox.js';
...
export class DebugGraphics {
    ...
    private drawFixtureShape(fixture: Fixture) {
        ...
        switch (fixture.shape) {
            ...
            case SHAPES.CIRCLE:
                ...
                this.ctx.lineTo(fx + Math.cos(fr) * fixture.radius * ANIMSCALE, fy + Math.sin(fr) * fixture.radius * ANIMSCALE);
                break;
            case SHAPES.AABB:
    ...
    private drawVectors(body: Body) {
        ...
        if (this.showForceVectors) {
            let fx = body.fx + body.nfx, fy = body.fy + body.nfy;
            let mag = Math.sqrt(fx * fx + fy * fy);
            if (mag > 0.001) {
                if (mag > MAX_VECTOR_MAGNITUDE) { fx = (fx / mag) * MAX_VECTOR_MAGNITUDE; fy = (fy / mag) * MAX_VECTOR_MAGNITUDE; mag = MAX_VECTOR_MAGNITUDE; }
        ...
        if (this.showImpulseVectors) {
            let ix = body.ix + body.nix, iy = body.iy + body.niy;
            let mag = Math.sqrt(ix * ix + iy * iy);
            if (mag > 0.001) {
                if (mag > MAX_VECTOR_MAGNITUDE) { ix = (ix / mag) * MAX_VECTOR_MAGNITUDE; iy = (iy / mag) * MAX_VECTOR_MAGNITUDE; mag = MAX_VECTOR_MAGNITUDE; }
    ...
    private drawLabels() {
        ...
        for (const label of this.labels) {
            ...
            if (label.objectId !== undefined) {
                ...
                if (body) {
                    ...
                    if (body.fixtures.length > 0) {
                        ...
                        if (f.shape === SHAPES.CIRCLE) { width = f.radius * 2 * ANIMSCALE; height = f.radius * 2 * ANIMSCALE; }
                        else { width = f.width * ANIMSCALE; height = f.height * ANIMSCALE; }
                    }
                }
            } else { x = (label.x || 0) * ANIMSCALE; y = (label.y || 0) * ANIMSCALE; }
            ...
            const textWidth = metrics.width, textHeight = parseInt(this.ctx.font) || 12;
            switch (pos) {
                case 'left': lx = x - width / 2 - textWidth / 2 - offset; break;
                case 'right': lx = x + width / 2 + textWidth / 2 + offset; break;
                case 'above': ly = y - height / 2 - textHeight / 2 - offset; break;
                case 'below': ly = ly + height / 2 + textHeight / 2 + offset; break;
            }
