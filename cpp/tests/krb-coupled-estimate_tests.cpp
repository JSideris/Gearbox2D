...
TEST(KrbCoupledEstimate, PostSolveDoesNotTouchStack) {
	World world;
	world.setGravity(0.0f, 10.0f);
	world.setTimeStep(1.0f / 60.0f);
...
TEST(KrbCoupledEstimate, PostSolveDoesNotTouchSpeculative) {
	World world;
	world.setGravity(0.0f, 10.0f);
	world.setTimeStep(1.0f / 60.0f);
...
TEST(KrbCoupledEstimate, PostSolveDoesNotTouchJointsOnlyPendulum) {
	World world;
	world.setGravity(0.0f, 10.0f);
	world.setTimeStep(1.0f / 60.0f);
	...
	Body* bob = world.getBody(2);
	ASSERT_NE(bob, nullptr);

	EXPECT_TRUE(std::isfinite(bob->getPosition().x));
	EXPECT_TRUE(std::isfinite(bob->getPosition().y));
	EXPECT_TRUE(std::isfinite(bob->getVelocity().x));
	EXPECT_TRUE(std::isfinite(bob->getVelocity().y));
