...
Evaluation is restricted to $e = 1$, zero friction/damping, sleep off (Section~\ref{sec:evaluation}); not a general frictional or $e < 1$ claim. No experimental Component A-only or Component B-only ablation was run; full KRB versus \texttt{-DGEARBOX\_DISABLE\_KRB} is the measured switch. Published traces therefore cannot attribute Dataset~A (or Dataset~D) residuals to Component~A versus~B; isolating the two leaks remains future work.
...
Dataset~C KRB-on enclosure stays in the box for $8\,\mathrm{h}$; $600\,\mathrm{s}$ windowed $E/E_0$ drifts $1.009 \to 0.977$ (slow loss, not the KRB-off SI gain of Figure~\ref{fig:ablation}b).
