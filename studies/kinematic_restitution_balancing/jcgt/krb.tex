...
Listings show Gearbox2D \texttt{preSolve} (penetration slop, \texttt{MAX\_POSITION\_CORRECTION}, cumulative $\Gamma$, and folding $-\mathrm{forceVn}$ into \texttt{bias}); a port \emph{merges} this audit into that engine's contact/joint setup rather than pasting the listing into another solver.
...
\small
\begin{table}[htb]
  \caption{Dataset~D: native KRB on/off wall-time per \texttt{world.step()} (mean and repeat range, ms/step). Gearbox2D ships as WASM; these timings are native \texttt{g++} recapture (\texttt{make log-timing}), not shipped WASM cost and not Gearbox-versus-Box2D CPU. Whole-step timings may include hinge Component~A (engine-only); Listings~\ref{lst:contact}--\ref{lst:joint} and the analytic op-count above cover contacts and distance joints only. High-pressure circle KRB-off escapes early (Dataset~A C02); on/off timings are not a same-contact-load comparison---do not read the spread as KRB overhead. \texttt{BENCH\_FLAGS}: \texttt{-O3 -march=native -mfma -pthread -DGEARBOX\_MT}; $100$ warmup + $1000$ timed steps, three repeats (i9-9900KF, g++ 13.3.0, \texttt{velocity\_iterations}$=50$, \texttt{position\_iterations}$=10$). Compile-time ablation; not Box2D. Traces: \texttt{data/timing/timing-summary-\{krb,nokrb\}.csv}.}
  \label{tab:dataset-d}
  \centering
  \begin{tabular}{@{}lrrl@{}}
    \hline
...
\textbf{Evaluation regime.} In scope: single-pass SI/PGS, symplectic Euler (Component~A not evaluated on other integrators), contacts and distance joints, $e = 1$, zero friction/damping, sleep off, and the Gearbox \texttt{World()}/\texttt{constants.h} knobs in Section~\ref{sec:evaluation}. Energy claims C01--C07 are reported only for this regime; friction, $e < 1$, other $dt$, or different iteration counts are out of scope. Dataset~A/C cradle energy includes rotational $E_k$; Dataset~B cradle $E/E_0$ is translational only---not the same meter as Figure~\ref{fig:ablation}c ($1.07$) versus Table~\ref{tab:dataset-b} Gearbox ($1.062$). Shown limits: coupled cradle $\sim 7\%$ step then $E/E_0 \in [1.034, 1.093]$ (mean $1.053$) through $8\,\mathrm{h}$ (Figures~\ref{fig:ablation}c, \ref{fig:dataset-c}a); C07 slow loss $1.009 \to 0.977$ (Figure~\ref{fig:dataset-c}b); speculative contacts omit Component~B; joint sqrt cap leaves resting sag; KRB-off enclosure escape (Figure~\ref{fig:ablation}b) is SI leak, not KRB failure; unmodified Box2D/p2/Matter (Figure~\ref{fig:engines}b) dissipate rather than gain. Out of scope: frictional or $e < 1$ games, hinges (engine-only), XPBD/TGS, global conservation. Variational integrators \cite{Hairer2006} are a different claim.
...
KRB is a drop-in \texttt{preSolve} audit for velocity-level SI solvers \cite{Catto2005}: Component~A compensates symplectic-Euler force drift at setup; Component~B taxes PE work of expected $\Delta h$, at a few scalars per constraint with zero extra PGS iterations (Table~\ref{tab:dataset-d}). Coupled scenes (C06) show a bounded $E/E_0$ offset after an early step, not full cradle compensation. KRB-on is not a perfect energy meter: Dataset~C enclosure (C07) drifts slowly ($1.009 \to 0.977$), i.e.\ loss not conservation and not the KRB-off SI gain of Dataset~A. The coupled-constraint gap in Section~\ref{sec:limitations} remains open.
