...
Newton's cradle is that coupled case. In Figure~\ref{fig:ablation}c and Figure~\ref{fig:engines}b the KRB run shows a $\sim 7\%$ step near four minutes, then a plateau---not secular climb, not a global invariant. Dataset~C (Figure~\ref{fig:dataset-c}a) continues KRB-on cradle to $8\,\mathrm{h}$ ($dt = 1/60$, sampled every $10\,\mathrm{s}$). After the step, $E/E_0$ stays in $[1.034, 1.093]$ (mean $1.053$) through $t = 28800\,\mathrm{s}$; $60\,\mathrm{s}$ windowed means do not jump again. Empirical multi-hour bound for this scene, not a global invariant.

Dataset~C KRB-on enclosure (Figure~\ref{fig:dataset-c}b) stays in the box for $8\,\mathrm{h}$; $600\,\mathrm{s}$ windowed $E/E_0$ drifts $1.009 \to 0.977$ (slow loss, not the KRB-off SI gain of Figure~\ref{fig:ablation}b).
...
Coupled-constraint audit is future work. Tunneling, ill-conditioned chains, and scenes failing for other reasons are outside the claim. The no-KRB enclosure escape (Figure~\ref{fig:ablation}b) is SI leak at high impact rate, not KRB failure. This note is denser than typical JCGT articles ($\sim 4$ pages) but within the venue $12$-page ceiling.

\section{Conclusion}

Kinematic Restitution Balancing is a drop-in \texttt{preSolve} audit for velocity-level SI solvers \cite{Catto2005}: Component~A compensates force-integration drift; Component~B taxes PE work of expected $\Delta h$, at a few scalars per constraint with zero extra PGS iterations (Table~\ref{tab:dataset-d}). The coupled-constraint gap in Section~\ref{sec:limitations} remains open.
