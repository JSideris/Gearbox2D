...
Figure~\ref{fig:ablation} is a compile-time ablation of the Gearbox2D sequential-impulse solver: default KRB on versus the same sources built with \texttt{-DGEARBOX\_DISABLE\_KRB}. Figure~\ref{fig:engines} places that KRB-on build next to unmodified Box2D-WASM, p2.js, and Matter.js. Dataset~B is whole-engine behavior (iteration counts, split impulse, restitution thresholds, and default damping differ); it is not an ablation of KRB inside those engines. Dataset~C, an $8\,\mathrm{h}$ KRB-on continuation of the cradle and a contact-only enclosure check, has no figure; traces are in \texttt{data/dataset-c/}. Numeric tables and limitations appear in later revisions.
